...
if(!function_exists(imp_users_visited_block_func))
{
	function imp_users_visited_block_func()
	{
		...
		for ($i = 0; $i < $counter; $i++)
		{
			...
			{
				$user_day_link = '<a href="' . append_sid("privmsg.php?mode=post&amp;" . POST_USERS_URL . "=" . $day_users_array[$i]['user_id']) . '"' . $style_color .' title="Send User a PM" onMouseOver=document.all.userscroll' . $scroll_num . '.stop() onMouseOut=document.all.userscroll' . $scroll_num . '.start()><img src="' . $images['scroll_pm'] . '" align=top border=0></a> <a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $day_users_array[$i]['user_id']) . '"' . $style_color .' title="View Users Profile" onMouseOver=document.all.userscroll' . $scroll_num . '.stop() onMouseOut=document.all.userscroll' . $scroll_num . '.start()>' . $day_users_array[$i]['username'] . '</a>';
			}
			else
			{
				$user_day_link = '<a href="' . append_sid("privmsg.php?mode=post&amp;" . POST_USERS_URL . "=" . $day_users_array[$i]['user_id']) . '"' . $style_color .' title="Send User a PM" onMouseOver=document.all.userscroll' . $scroll_num . '.stop() onMouseOut=document.all.userscroll' . $scroll_num . '.start()><img src="' . $images['scroll_pm'] . '" align=top border=0></a> <a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $day_users_array[$i]['user_id']) . '"' . $style_color .' title="View Users Profile" onMouseOver=document.all.userscroll' . $scroll_num . '.stop() onMouseOut=document.all.userscroll' . $scroll_num . '.start()><i>' . $day_users_array[$i]['username'] . '</i></a>';
			}
		...
		$day_userlist = ( ( isset($forum_id) ) ? '' : sprintf($lang['Day_users'], $day_users, $portal_config['md_hours_track_users']) ) . '<br /><marquee scrolldelay=' . $portal_config['md_scroll_delay'] . ' id=userscroll1>' . $day_userlist . '</marquee>';

		$not_day_userlist = ( ( isset($forum_id) ) ? '' : sprintf($lang['Not_day_users'], $not_day_users, $portal_config['md_hours_track_users']) ) . '<br /><marquee scrolldelay=' . $portal_config['md_scroll_delay'] . ' id=userscroll2>' . $not_day_userlist . '</marquee>';

		if ( $portal_config['md_display_not_visit'] )
		{
			$day_userlist .= '<br />' . $not_day_userlist;
		}
		$template->assign_var('FEROZ_DAY_LIST' , $day_userlist);
	}
}
